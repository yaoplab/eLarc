"""Panel Publicité Web."""
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QFileDialog, QTextEdit
from PySide6.QtCore import Qt
from phibuilder.widgets import M3Label, M3ScrollArea, M3ComboBox, M3Button, M3Frame
from phibuilder.widgets.button import ButtonVariant
from phibuilder.phi.scale import SpacingToken
from larccommon.theme import theme_manager
from larccommon.icons import icon as md3_icon
from LarcDocs.common.templates import get_app_list, gen_web_ad
from LarcDocs.common.exporter import export_html, export_pdf_from_html, copy_to_clipboard


class AdWebPanel(M3ScrollArea):
    def __init__(self, user: dict):
        ...
        l.addWidget(M3Label("Publicité Web", theme=phi, style="headline_small"))
        l.addWidget(M3Label("Générer une landing page HTML pour une application Larc.",
        ...
        l.addWidget(M3Label("Aperçu HTML :", theme=phi, style="title_small"))
        self._preview = QTextEdit()
        self._preview.setReadOnly(False)
        self._preview.setMinimumHeight(300)
        self._preview.setStyleSheet(
            f"QTextEdit {{ background: {c.surface}; color: {c.on_surface}; "
            f"border: 1px solid {c.outline}; border-radius: 8px; padding: 12px; }}")
        ...
        for label_text, icon_name, slot in [
            ("Exporter .html", 'save', self._export_html),
            ("Aperçu navigateur", 'visibility', self._preview_browser),
            ("Copier HTML", 'subject', self._copy),
        ]:
    ...
    def _generate(self):
        ...
        self._generated = gen_web_ad(app)
        self._preview.setPlainText(self._generated)

    def _export_html(self):
        text = self._preview.toPlainText() or self._generated
        path, _ = QFileDialog.getSaveFileName(self, "Exporter HTML", "landing_page.html",
                                               "HTML (*.html);;Tous (*.*)")
        if path:
