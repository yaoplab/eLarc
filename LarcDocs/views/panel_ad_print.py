...
from LarcDocs.common.templates import get_app_list, gen_print_ad, gen_print_ad_latex
from LarcDocs.common.exporter import (export_markdown, export_pdf_from_html,
                                       copy_to_clipboard, export_latex, compile_latex,
                                       check_latex_available)


class AdPrintPanel(M3ScrollArea):
    def __init__(self, user: dict):
        ...
        l.addWidget(M3Label("Publicité Imprimable", theme=phi, style="headline_small"))
        l.addWidget(M3Label("Générer un document imprimable (brochure, flyer) pour une application Larc.",
    ...
    def _generate(self):
        ...
        self._generated = gen_print_ad(app)
        self._preview.setPlainText(self._generated)

    def _export_md(self):
        text = self._preview.toPlainText() or self._generated
        path, _ = QFileDialog.getSaveFileName(self, "Exporter Markdown", "brochure.md",
                                               "Markdown (*.md);;Tous (*.*)")
        if path:
    ...
    def _export_latex(self):
        import os
        app = self._app_combo.currentText()
        latex_content = gen_print_ad_latex(app)
        path, _ = QFileDialog.getSaveFileName(self, "Exporter LaTeX", "brochure.tex",
                                               "LaTeX (*.tex);;Tous (*.*)")
        if not path:
            return
