...
from .database import DBMode, db
...
def _deduce_role(is_adm: bool, is_coordonator: bool, is_secretary: bool, is_sup: bool = False) -> UserRole:
    if is_adm:
        return UserRole.ADMIN
    if is_coordonator:
        return UserRole.COORD
    if is_secretary:
        return UserRole.SECR
    if is_sup:
    ...
    def auth_intranet(cls, email: str, password: str) -> Tuple[bool, AuthResult, str]:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT id, email, last_name, first_name, password FROM larcauth_aecuser WHERE LOWER(email) = %s", (email.strip().lower(),))
                row = cur.fetchone()
            ...
            with conn.cursor() as cur:
                cur.execute("SELECT type_director, type_coordonator, type_secretary, type_supervisor FROM larcauth_aecuser WHERE id = %s", (user_id,))
                roles = cur.fetchone()
                if roles is None or not any(roles):
                    return False, AuthResult(), "Aucun rôle trouvé"

            role = _deduce_role(is_adm=bool(roles[0]), is_coordonator=bool(roles[1]), is_secretary=bool(roles[2]), is_sup=bool(roles[3]))
...
class OAuth2Manager:
    ...
    def authenticate(cls) -> Tuple[bool, AuthResult, str]:
        ...
        try:
            req = urllib.request.Request(cls.GOOGLE_TOKEN, data=token_body, method="POST", headers={"Content-Type": "application/x-www-form-urlencoded"})
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT id, first_name, last_name FROM public.larcauth_aecuser WHERE LOWER(email) = %s", (email.lower(),))
                row = cur.fetchone()
            if row is None:
