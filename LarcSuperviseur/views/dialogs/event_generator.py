...
class EventGenerator(ThemedDialog):
    ...
    def _init_ui(self):
        ...
        self._ni.setMaxLength(200)
        self._ni.setAccessibleName(_("event.note"))
        self._ni.setToolTip(_("event.note_tooltip"))
    ...
    def _show_context_buttons(self):
        """N1 Événement : 'En salle de cours' + les autres lieux (pas des nœuds DB).

        'En salle de cours' → matières ; un autre lieu → types directement
        (le lieu est déjà choisi ici).
        """
        h = ds.space_xl * 2  # 104px
        b = M3Button(_("event_types.in_class"), variant=ButtonVariant.TONAL)
        b.setMinimumHeight(h)
        b.setCursor(Qt.PointingHandCursor)
        b.clicked.connect(
            lambda checked, l=_("event_types.in_class"): self._on_step_click(l, context="cours")
        )
        self._step_grid.addWidget(b, 0, 0)
        # Les autres lieux (salle de cours exclue — gérée par le bouton dédié)
        idx = 1
        for lid, _sid, lieu_name in self._locations:
            if lieu_name.strip().lower() == "salle de cours":
                continue
            bb = M3Button(lieu_name, variant=ButtonVariant.OUTLINED)
            bb.setMinimumHeight(ds.space_xl - ds.space_xs)  # 44px
            bb.setCursor(Qt.PointingHandCursor)
            bb.clicked.connect(
                lambda checked, l=lieu_name, lid=lid: self._on_step_click(
                    l, context="hors", lieu_id=lid, lieu_label=l
                )
            )
            self._step_grid.addWidget(bb, idx // 3, idx % 3)
            idx += 1

    def _show_context_pick(self):
        """N2 Événement : matières (En salle de cours) ; sinon types (lieu choisi)."""
